...
import motor_driver
state = 1
driver_left = motor_driver.Controller("/dev/right_wheel", 7)
driver_right = motor_driver.Controller("/dev/left_wheel", 7)
...
def main():
    ...
    try:
        while True:
            if state == 1:
                encoder_ticks_left_start = driver_left.get_position_raw()
                encoder_ticks_right_start = driver_right.get_position_raw()
                ...
                encoder_ticks_left = driver_left.get_position_raw()
                encoder_ticks_right = driver_right.get_position_raw()
